...
# 日志文件路径
readonly LOG_FILE="/var/log/sing-box-script.log"
...
error_handler() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        log_error "命令: $last_cmd"
        log_error "调用栈: ${func_stack//::/ -> }"
        log_error "============================"
    fi
}
...
# 加载模块化组件
# 注意: 如果模块文件存在则加载,否则使用core.sh中的旧函数
if [[ -f "$is_sh_dir/src/loader.sh" ]]; then
    msg "加载模块化组件..."
    # 设置环境变量,让loader自动加载所有协议
