...
install_package() {
    ...
    if command -v apt-get >/dev/null 2>&1; then
        ...
        log_error "无法确定包管理器，请手动安装 $package"
        return 1
    fi
    ...
    case "$archive_pattern" in
        *.zip)
            if ! command -v unzip > /dev/null 2>&1; then
                log_warn "未找到 unzip 工具，尝试自动安装..."
                if ! install_package "unzip"; then
                    log_error "unzip 安装失败，虽然我们尽力了。请手动安装 unzip 后重试。"
                    rm -rf "$temp_dir"
                    return 1
                fi
            fi
...
if [ ! -e "$HOME/agsbx/sbk/private_key" ] || [ ! -s "$HOME/agsbx/sbk/private_key" ]; then
...
if [ -n "$ssp" ]; then
ssp=sspt
if [ ! -e "$HOME/agsbx/sskey" ] || [ ! -s "$HOME/agsbx/sskey" ]; then
...
port_vm_ws=$(cat "$HOME/agsbx/port_vm_ws")
vm_link="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${sxname}vm-xhttp-$hostname\", \"add\": \"$server_ip\", \"port\": \"$port_vm_ws\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"xhttp\", \"type\": \"http\", \"host\": \"www.bing.com\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
...
echo "注：默认地址 yg数字.ygkkk.dpdns.org 可自行更换优选IP域名，如是回源端口需手动修改443或者80系端口"
vm_cdn_link="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${sxname}vm-xhttp-cdn-$hostname\", \"add\": \"yg$(cfip).ygkkk.dpdns.org\", \"port\": \"$port_vm_ws\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"xhttp\", \"type\": \"http\", \"host\": \"$xvvmcdnym\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
...
hy2_link="hysteria2://$uuid@$server_ip:$port_hy2?security=tls&alpn=h3&insecure=1&sni=$random_cn#${sxname}hy2-$hostname"
...
port_tu=$(cat "$HOME/agsbx/port_tu")
random_cn=$(cat "$HOME/agsbx/cert_cn" 2>/dev/null || echo "www.bing.com")
tuic5_link="tuic://$uuid:$uuid@$server_ip:$port_tu?congestion_control=bbr&udp_relay_mode=native&alpn=h3&sni=$random_cn&allow_insecure=1&allowInsecure=1#${sxname}tuic-$hostname"
...
# echo -e "nameserver 2a00:1098:2b::1\nnameserver 2a00:1098:2c::1" > /etc/resolv.conf
    log_info "提示: 为了更好的连接体验，建议检查 DNS 设置 (/etc/resolv.conf)"
fi
...
echo
echo "========================================================="
echo " 重要提示：脚本已停止自动修改系统防火墙/SELinux设置 "
echo " 请根据下方的端口信息，自行在防火墙开放对应端口 "
echo "========================================================="
ins
cip
# Function to generate ports.conf and list used ports
...
EOF
    ...
    add_port() {
        ...
        if [ -f "$HOME/agsbx/$file" ]; then
            ...
            # If proto is tcp/udp, we might want to list both lines or just one?
            # User example: tcp:8443. 
            # If it's tcp/udp, we probably should specify it clearly.
            # For simplicity and compliance with common firewalls, if it's dual stack:
...
echo
echo "========================================================="
echo " 重要提示：请务必在云服务器控制台防火墙/安全组放行端口 "
echo "========================================================="
exit
fi
